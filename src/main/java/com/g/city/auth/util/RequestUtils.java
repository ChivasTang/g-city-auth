...
public class RequestUtils {
    ...
    public static boolean isRegisterUrl(HttpServletRequest request) {
        return request.getRequestURI().contains(RouterConstants.REGISTER_ROUTER_MARKER) && StringUtils.equals(request.getMethod(), HttpMethod.POST.name());
    }
    ...
    public static boolean hasBearer(HttpServletRequest request) {
        ...
        return !StringUtils.isEmpty(requestTokenHeader)
                ...
                && !StringUtils.isEmpty(requestTokenHeader.substring(BEARER_HEADER_PREFIX.length()).trim());
    }
